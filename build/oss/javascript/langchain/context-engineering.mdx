...
|-------------|---------------|-------|----------|
| **Runtime Context** | Static configuration | Conversation-scoped | User ID, database connections, permissions, environment settings |
...
Most real-world tools need more than just the LLM's parameters. They need user IDs for database queries, or current session state to make decisions. Tools read from state, store, and runtime context to access this information.
...
<Tabs>
  ...
  <Tab title="Runtime Context">
    Read from Runtime Context for configuration like user IDs:
    ...
    const fetchUserData = tool(
      async ({ query }, runtime: ToolRuntime<any, typeof contextSchema>) => {
        ...
        const results = await performDatabaseQuery(dbConnection, query);
