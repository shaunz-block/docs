...
<Tabs>
  ...
  <Tab title="Runtime Context">
    Read from Runtime Context for configuration like API keys and user IDs. Prefix sensitive fields with `__` to prevent them from being traced:
    ...
    const contextSchema = z.object({
      userId: z.string(),
      __apiKey: z.string(),  // __ prefix: excluded from tracing
    ...
    const fetchUserData = tool(
      async ({ query }, runtime: ToolRuntime<any, typeof contextSchema>) => {
        ...
        const { userId, __apiKey, dbConnection } = runtime.context;

        // Use configuration to fetch data
        const results = await performDatabaseQuery(dbConnection, query, __apiKey);
...
<Warning>
Prefix any sensitive Runtime Context field with `__` (double underscore) to exclude it from tracing and prevent it from being exposed to the client. This applies to API keys, tokens, credentials, and any other data you want to keep out of traces. For example, use `__api_key` instead of `api_key`.
</Warning>
