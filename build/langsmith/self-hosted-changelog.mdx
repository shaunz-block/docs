...
- Added capability for users to find account labels for first-class providers for improved user navigation.
- Fixed the issue where switching alerts did not change the chart display.
- Fixed a KeyError issue for experiments with attachments to ensure stable performance.
- Added a dismiss button to the agentify banner for better user control.
- Improved responsiveness in the run details header and trace comparison for enhanced user experience.
- Added a truncate property to feedback chips list to be responsive to container width.
- Covered subpixel bleed on body rows in the repetition summary table for visual consistency.
- Fixed an issue with race conditions during run archive checks to improve reliability.
- Patched multiple security alerts to enhance overall platform security.
- Renamed 'Metadata' to 'Attributes' in comparison detail pane for clarity.
- Enabled the toggle panel size button in the RepetitionDetailPane.
- Updated model cards in the frontend for better model information access.
- Added MCP server RBAC permission constants to improve authorization checks.
- Fixed loading of the prompt picker when opening the playground from the experiments table.
- Properly disabled environment promotion buttons for users without tag permissions.
- Enabled granular usage rollup cron for self-hosted deployments to provide detailed usage analytics.
- Increased auditing of logs with more CUD operations to enhance traceability.
- Added Ashby integration and tools in the Agent Builder for more integration options.
- Demonstrated a proof of concept for the LLM gateway, expanding connectivity.
- Improved performance in the Agent Builder by parallelizing graph loading.
- Added Dynatrace webhook integration for expanded alerting capabilities.
- Showed a toast notification on clicking the run now button in the forge for user feedback.
- Fixed issues with non-string resource fields in MCP OAuth discovery for seamless integration.
- Added a feedback banner for experiment sidebar redesign to gather user inputs.
- Added example attachments to experiment detail panes for better experiment insights.
- Wired the arcade integration to real OAuth flow for secure authentication.
- Prevented flash loading in host revisions table during revalidation for a smoother experience.
- Fixed imports in the host backend related to truststore to enhance security.
- Added request context to authentication middleware error logs for better error tracking.
- Improved editing capabilities of prompts in the evaluator for user flexibility.
- Fetched full run data in the new experiment detail pane to provide comprehensive information.
